...
from policy import SamplingPolicy, get_example_time_policy, get_example_geo_policy
...
def produce_answer_at_level_time(query, level, gt_day, gt_month, gt_year, gt_decade):

    _, l_day, l_month, l_year, l_decade = recover_time_params(query, time_template)
    if level == 0:
        if l_day == gt_day and l_day is not None:
            return True
        else:
            return False

    if level == 1:
        if l_month == gt_month and l_month is not None:
            return True
...
def build_gt_time_correct_set(correct_string, retrieval_set, correctness_level, time_template):

    _, gt_day, gt_month, gt_year, gt_decade = recover_time_params(correct_string, time_template)


    correct_elements = []
    for each in retrieval_set:

        if produce_answer_at_level_time(each[0], correctness_level, gt_day, gt_month, gt_year, gt_decade):
            correct_elements.append(each)

    return correct_elements

def produce_answer_at_level_geo(query, level, gt_full_string, gt_country, gt_continent):
...
def build_gt_geo_correct_set(correct_string, retrieval_set, correctness_level):
    ...
    for each in retrieval_set:
        ...
        if produce_answer_at_level_geo(each[0], correctness_level, gt_full_string, gt_country, gt_continent):
            correct_elements.append(each)
...
class GeoTemporalDataset(data.Dataset):
    ...
    def __init__(self, model_name, exp_type, data_dir='input_zip_nyt_dataset', template_options=(0, 0), ratio=10,
                correctness_level=0, mode='test', sample=False):
        ...
        try:
            isinstance(self.correctness_levels, int)
        except:
            raise Exception("The type for correctness level was a string, pls pass in an integer or check your code")
    ...
    def template_time_label(self, time_label, idx=0, basic_time_string=None):
        ...
        '''
        template = "This picture was taken %s"
    ...
    def sample_from_full_negatives(self, number, forbidden_set, correct_string=None):
        ...
        if forbidden_set:
            ...
            while len(all_negatives_final) != number:
                ...
                if self.exp_type == 1:
                    ...
                    current_val = filter(lambda each: not produce_answer_at_level_time(each[0], self.correctness_levels,
                                                                                       gt_day, gt_month, gt_country, gt_continent))
                ...
                if self.exp_type == 2:
                    ...
                    current_val = filter(lambda each: not produce_answer_at_level_geo(each[0], self.correctness_levels, \
                                                                    gt_full_string, gt_country, gt_continent), current_val)
            ...
            return set(random.sample(all_negatives, number))
    ...
    def __getitem__(self, main_idx):
        ...
        if self.exp_type == 1:
            ...
            correct_string = self.template_time_label(main_info, idx=data_obj.info_base_level)
            retrieval_set.add((self.template_time_label(main_info, idx=data_obj.info_base_level), data_obj.info_base_level))
            ...
            for idx in range(1, len(all_positives)+1):
                info_set.add(all_positives[idx-1])
            ...
            if self.sample:
                sampled_negative_set = self.sample_from_full_negatives(19 * len(correct_elements), retrieval_set_with_pos_only, correct_string)
            ...
            #correct_elements = build_gt_time_correct_set(correct_string, retrieval_set, self.correctness_levels, self.time_template_options)
            ...
            for idx in range(0, len(all_positives)):
                info_set.add(all_positives[idx][0])
            ...
            if self.sample:
                sampled_negative_set = self.sample_from_full_negatives(19 * len(correct_elements),
                                                                       retrieval_set_with_pos_only, correct_string)
        ...
        tokenized_text = torch.stack([self.tokenize_fn(sentence[0][:64]) for sentence in retrieval_set]).squeeze()


        if correct_elements:
            tokenized_correct_text = torch.stack([self.tokenize_fn(sentence[0][:64]) for sentence in correct_elements]).squeeze()

        else:
            tokenized_correct_text = torch.Tensor([-1])

        #TODO check if added line is correct
        correct_string_tokenized = self.tokenize_fn(correct_string[:64])

        text_types = torch.Tensor([sentence[1] for sentence in retrieval_set])

        if correct_elements:
            correct_text_types = torch.Tensor([sentence[1] for sentence in correct_elements])

        else:
            correct_text_types = torch.Tensor([-1])


        if self.mode=='train':
            return {'img': img.to('cpu'), 'correct_string_tokens': correct_string_tokenized}
            ...
            return {'idx': main_idx,
                ...
                'gt_data_url': data_obj.img_url,
                'correct_string': correct_string,
                'correct_string_tokens': correct_string_tokenized,
                'root_img_path': img_file_path,
                'root_file': self.idx_linking[main_idx][0], #for debugging, cross-checking
                'root_file_line': self.idx_linking[main_idx][1], #for debugging, cross-checking
                'full_correct_set': correct_elements,
                'tokenized_text': tokenized_text.to('cpu'),
                'correct_indices': torch.Tensor(correct_indices),
                'text_types': text_types.to('cpu'),
                'tokenized_correct': tokenized_correct_text.to('cpu'),
                'correct_text_types': correct_text_types.to('cpu'),
                'full_pos_set': list(retrieval_set_with_pos_only)}
...
def test_file():
    ...
    zero_shot_dataset.__getitem__(234)
